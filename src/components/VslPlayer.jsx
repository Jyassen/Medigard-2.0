...
export default function VslPlayer({
  ...
  return (
    <div
      className={`rounded-[2.5rem] shadow-2xl border-8 border-white overflow-hidden bg-slate-900 ${className}`}
    >
      {wistiaId ? (
      ...
      {(kicker || title) && (
        <div className="px-6 py-4 bg-slate-900 border-t border-slate-800 text-left">
          ...
          {title && (
            <h3
              className={`text-white text-xl md:text-2xl font-black ${titleClassName}`.trim()}
            >
              {title}
            </h3>
          )}
